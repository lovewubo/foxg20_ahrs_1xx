...
static real_T c18_emlrt_marshallIn(SFc18_EKFInstanceStruct *chartInstance, const
  mxArray *c18_psi, const char_T *c18_identifier);
...
static const mxArray *sf_get_sim_state_info_c18_EKF(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"psi\",},{M[8],M[0],T\"is_active_c18_EKF\",}}"
  };
...
static void mdlSetWorkWidths_c18_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"EKF","EKF",18,3);
