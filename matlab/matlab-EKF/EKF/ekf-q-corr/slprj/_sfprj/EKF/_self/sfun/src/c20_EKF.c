...
static void initialize_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void initialize_params_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void enable_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void disable_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void c20_update_debugger_state_c20_EKF(SFc20_EKFInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c20_EKF(SFc20_EKFInstanceStruct
  *chartInstance);
static void set_sim_state_c20_EKF(SFc20_EKFInstanceStruct *chartInstance, const
  mxArray *c20_st);
static void finalize_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void sf_c20_EKF(SFc20_EKFInstanceStruct *chartInstance);
static void initSimStructsc20_EKF(SFc20_EKFInstanceStruct *chartInstance);
...
static const mxArray *c20_sf_marshallOut(void *chartInstanceVoid, void
  *c20_inData);
static real_T c20_emlrt_marshallIn(SFc20_EKFInstanceStruct *chartInstance, const
  mxArray *c20_psi, const char_T *c20_identifier);
static real_T c20_b_emlrt_marshallIn(SFc20_EKFInstanceStruct *chartInstance,
...
static const mxArray *c20_c_sf_marshallOut(void *chartInstanceVoid, void
  *c20_inData);
static int32_T c20_c_emlrt_marshallIn(SFc20_EKFInstanceStruct *chartInstance,
  const mxArray *c20_u, const emlrtMsgIdentifier *c20_parentId);
static void c20_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c20_mxArrayInData, const char_T *c20_varName, void *c20_outData);
static uint8_T c20_d_emlrt_marshallIn(SFc20_EKFInstanceStruct *chartInstance,
  const mxArray *c20_b_is_active_c20_EKF, const char_T *c20_identifier);
static uint8_T c20_e_emlrt_marshallIn(SFc20_EKFInstanceStruct *chartInstance,
  const mxArray *c20_u, const emlrtMsgIdentifier *c20_parentId);
static void init_dsm_address_info(SFc20_EKFInstanceStruct *chartInstance);
...
static void initialize_c20_EKF(SFc20_EKFInstanceStruct *chartInstance)
{
  ...
  chartInstance->c20_is_active_c20_EKF = 0U;
}
  ...
  *chartInstance)
{
  ...
  c20_b_hoistedGlobal = chartInstance->c20_is_active_c20_EKF;
  ...
  mxArray *c20_st)
{
  ...
  *c20_psi = c20_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c20_u,
    0)), "psi");
  chartInstance->c20_is_active_c20_EKF = c20_d_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c20_u, 1)), "is_active_c20_EKF");
  sf_mex_destroy(&c20_u);
  c20_update_debugger_state_c20_EKF(chartInstance);
  ...
  *c20_inData)
{
  ...
  real_T c20_u;
  const mxArray *c20_y = NULL;
  SFc20_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc20_EKFInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c20_EKF_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c20_b_is_active_c20_EKF, const char_T *c20_identifier)
{
  ...
  c20_y = c20_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c20_b_is_active_c20_EKF), &c20_thisId);
  sf_mex_destroy(&c20_b_is_active_c20_EKF);
  return c20_y;
}
...
/* SFunction Glue Code */
void sf_c20_EKF_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c20_EKF_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c20_EKF(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"psi\",},{M[8],M[0],T\"is_active_c20_EKF\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c20_EKF_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc20_EKFInstanceStruct *chartInstance;
    chartInstance = (SFc20_EKFInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c20_EKF(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc20_EKFInstanceStruct*) chartInstanceVar)->S,0);
  initialize_params_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
  initialize_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c20_EKF(void *chartInstanceVar)
{
  enable_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c20_EKF(void *chartInstanceVar)
{
  disable_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c20_EKF(void *chartInstanceVar)
{
  sf_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c20_EKF(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c20_EKF((SFc20_EKFInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c20_EKF();/* state var info */
...
extern void sf_internal_set_sim_state_c20_EKF(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c20_EKF((SFc20_EKFInstanceStruct*)chartInfo->chartInstance,
                        mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c20_EKF(SimStruct* S)
{
  return sf_internal_get_sim_state_c20_EKF(S);
}

static void sf_opaque_set_sim_state_c20_EKF(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c20_EKF(S, st);
}

static void sf_opaque_terminate_c20_EKF(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc20_EKFInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc20_EKF((SFc20_EKFInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c20_EKF(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c20_EKF((SFc20_EKFInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c20_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"EKF","EKF",20);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"EKF","EKF",20,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"EKF","EKF",20,
      "gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"EKF","EKF",20,3);
      sf_mark_chart_reusable_outputs(S,"EKF","EKF",20,1);
    }

    sf_set_rtw_dwork_info(S,"EKF","EKF",20);
...
static void mdlRTW_c20_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "EKF", "EKF",20);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c20_EKF(SimStruct *S)
{
  SFc20_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc20_EKFInstanceStruct *)malloc(sizeof
    (SFc20_EKFInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc20_EKFInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c20_EKF;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c20_EKF;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c20_EKF;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c20_EKF;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c20_EKF;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c20_EKF;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c20_EKF;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c20_EKF;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c20_EKF;
  chartInstance->chartInfo.mdlStart = mdlStart_c20_EKF;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c20_EKF;
...
void c20_EKF_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   ...
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c20_EKF(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c20_EKF(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c20_EKF_method_dispatcher.\n"
