...
static void initialize_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void initialize_params_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void enable_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void disable_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void c2_update_debugger_state_c2_EKF(SFc2_EKFInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void set_sim_state_c2_EKF(SFc2_EKFInstanceStruct *chartInstance, const
  mxArray *c2_st);
static void finalize_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void sf_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void c2_chartstep_c2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void initSimStructsc2_EKF(SFc2_EKFInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c2_machineNumber, uint32_T
  c2_chartNumber);
static const mxArray *c2_sf_marshallOut(void *chartInstanceVoid, void *c2_inData);
static void c2_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance, const
  mxArray *c2_F, const char_T *c2_identifier, real_T c2_y[49]);
static void c2_b_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance, const
  mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId, real_T c2_y[49]);
...
static const mxArray *c2_c_sf_marshallOut(void *chartInstanceVoid, void
  *c2_inData);
static real_T c2_c_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance, const
  mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void c2_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c2_mxArrayInData, const char_T *c2_varName, void *c2_outData);
static const mxArray *c2_d_sf_marshallOut(void *chartInstanceVoid, void
  *c2_inData);
static int32_T c2_d_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void c2_c_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c2_mxArrayInData, const char_T *c2_varName, void *c2_outData);
static uint8_T c2_e_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance,
  const mxArray *c2_b_is_active_c2_EKF, const char_T *c2_identifier);
static uint8_T c2_f_emlrt_marshallIn(SFc2_EKFInstanceStruct *chartInstance,
  const mxArray *c2_u, const emlrtMsgIdentifier *c2_parentId);
static void init_dsm_address_info(SFc2_EKFInstanceStruct *chartInstance);
...
static void initialize_c2_EKF(SFc2_EKFInstanceStruct *chartInstance)
{
  ...
  chartInstance->c2_is_active_c2_EKF = 0U;
}
...
static const mxArray *get_sim_state_c2_EKF(SFc2_EKFInstanceStruct *chartInstance)
{
  ...
  c2_hoistedGlobal = chartInstance->c2_is_active_c2_EKF;
  ...
  mxArray *c2_st)
{
  ...
  chartInstance->c2_is_active_c2_EKF = c2_e_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c2_u, 1)), "is_active_c2_EKF");
  sf_mex_destroy(&c2_u);
  c2_update_debugger_state_c2_EKF(chartInstance);
...
static void sf_c2_EKF(SFc2_EKFInstanceStruct *chartInstance)
{
  ...
  chartInstance->c2_sfEvent = CALL_EVENT;
  c2_chartstep_c2_EKF(chartInstance);
...
static const mxArray *c2_sf_marshallOut(void *chartInstanceVoid, void *c2_inData)
{
  ...
  real_T c2_u[49];
  const mxArray *c2_y = NULL;
  SFc2_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc2_EKFInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c2_EKF_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c2_b_is_active_c2_EKF, const char_T *c2_identifier)
{
  ...
  c2_thisId.fIdentifier = c2_identifier;
  c2_thisId.fParent = NULL;
  c2_y = c2_f_emlrt_marshallIn(chartInstance, sf_mex_dup(c2_b_is_active_c2_EKF),
    &c2_thisId);
  sf_mex_destroy(&c2_b_is_active_c2_EKF);
  return c2_y;
}
...
/* SFunction Glue Code */
void sf_c2_EKF_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_EKF_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c2_EKF(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"F\",},{M[8],M[0],T\"is_active_c2_EKF\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c2_EKF_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc2_EKFInstanceStruct *chartInstance;
    chartInstance = (SFc2_EKFInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c2_EKF(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc2_EKFInstanceStruct*) chartInstanceVar)->S,0);
  initialize_params_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
  initialize_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c2_EKF(void *chartInstanceVar)
{
  enable_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c2_EKF(void *chartInstanceVar)
{
  disable_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c2_EKF(void *chartInstanceVar)
{
  sf_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c2_EKF(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c2_EKF((SFc2_EKFInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c2_EKF();/* state var info */
...
extern void sf_internal_set_sim_state_c2_EKF(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c2_EKF((SFc2_EKFInstanceStruct*)chartInfo->chartInstance,
                       mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c2_EKF(SimStruct* S)
{
  return sf_internal_get_sim_state_c2_EKF(S);
}

static void sf_opaque_set_sim_state_c2_EKF(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c2_EKF(S, st);
}

static void sf_opaque_terminate_c2_EKF(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc2_EKFInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc2_EKF((SFc2_EKFInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_EKF(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c2_EKF((SFc2_EKFInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c2_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"EKF","EKF",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"EKF","EKF",2,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"EKF","EKF",2,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"EKF","EKF",2,4);
      sf_mark_chart_reusable_outputs(S,"EKF","EKF",2,1);
    }

    sf_set_rtw_dwork_info(S,"EKF","EKF",2);
...
static void mdlRTW_c2_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "EKF", "EKF",2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_EKF(SimStruct *S)
{
  SFc2_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc2_EKFInstanceStruct *)malloc(sizeof(SFc2_EKFInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc2_EKFInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c2_EKF;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c2_EKF;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c2_EKF;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c2_EKF;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c2_EKF;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c2_EKF;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c2_EKF;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c2_EKF;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c2_EKF;
  chartInstance->chartInfo.mdlStart = mdlStart_c2_EKF;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_EKF;
...
void c2_EKF_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   ...
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_EKF(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_EKF(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c2_EKF_method_dispatcher.\n"
