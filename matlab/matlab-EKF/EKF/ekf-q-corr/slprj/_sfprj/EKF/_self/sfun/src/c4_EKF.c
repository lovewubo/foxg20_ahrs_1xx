...
static void initialize_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void initialize_params_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void enable_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void disable_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void c4_update_debugger_state_c4_EKF(SFc4_EKFInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void set_sim_state_c4_EKF(SFc4_EKFInstanceStruct *chartInstance, const
  mxArray *c4_st);
static void finalize_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void sf_c4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void initSimStructsc4_EKF(SFc4_EKFInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c4_machineNumber, uint32_T
  c4_chartNumber);
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData);
static void c4_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance, const
  mxArray *c4_dotxk, const char_T *c4_identifier, real_T c4_y[7]);
static void c4_b_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance, const
  mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId, real_T c4_y[7]);
static void c4_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static const mxArray *c4_b_sf_marshallOut(void *chartInstanceVoid, void
  *c4_inData);
static real_T c4_c_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance, const
  mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static const mxArray *c4_c_sf_marshallOut(void *chartInstanceVoid, void
  *c4_inData);
static int32_T c4_d_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance,
  const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_c_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static uint8_T c4_e_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance,
  const mxArray *c4_b_is_active_c4_EKF, const char_T *c4_identifier);
static uint8_T c4_f_emlrt_marshallIn(SFc4_EKFInstanceStruct *chartInstance,
  const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void init_dsm_address_info(SFc4_EKFInstanceStruct *chartInstance);
...
static void initialize_c4_EKF(SFc4_EKFInstanceStruct *chartInstance)
{
  ...
  chartInstance->c4_is_active_c4_EKF = 0U;
}
...
static const mxArray *get_sim_state_c4_EKF(SFc4_EKFInstanceStruct *chartInstance)
{
  ...
  c4_hoistedGlobal = chartInstance->c4_is_active_c4_EKF;
  ...
  mxArray *c4_st)
{
  ...
  chartInstance->c4_is_active_c4_EKF = c4_e_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c4_u, 1)), "is_active_c4_EKF");
  sf_mex_destroy(&c4_u);
  c4_update_debugger_state_c4_EKF(chartInstance);
...
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData)
{
  ...
  real_T c4_u[7];
  const mxArray *c4_y = NULL;
  SFc4_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc4_EKFInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c4_EKF_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c4_b_is_active_c4_EKF, const char_T *c4_identifier)
{
  ...
  c4_thisId.fIdentifier = c4_identifier;
  c4_thisId.fParent = NULL;
  c4_y = c4_f_emlrt_marshallIn(chartInstance, sf_mex_dup(c4_b_is_active_c4_EKF),
    &c4_thisId);
  sf_mex_destroy(&c4_b_is_active_c4_EKF);
  return c4_y;
}
...
/* SFunction Glue Code */
void sf_c4_EKF_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c4_EKF_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c4_EKF(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"dotxk\",},{M[8],M[0],T\"is_active_c4_EKF\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c4_EKF_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc4_EKFInstanceStruct *chartInstance;
    chartInstance = (SFc4_EKFInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c4_EKF(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc4_EKFInstanceStruct*) chartInstanceVar)->S,0);
  initialize_params_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
  initialize_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c4_EKF(void *chartInstanceVar)
{
  enable_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c4_EKF(void *chartInstanceVar)
{
  disable_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c4_EKF(void *chartInstanceVar)
{
  sf_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c4_EKF(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c4_EKF((SFc4_EKFInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c4_EKF();/* state var info */
...
extern void sf_internal_set_sim_state_c4_EKF(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c4_EKF((SFc4_EKFInstanceStruct*)chartInfo->chartInstance,
                       mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c4_EKF(SimStruct* S)
{
  return sf_internal_get_sim_state_c4_EKF(S);
}

static void sf_opaque_set_sim_state_c4_EKF(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c4_EKF(S, st);
}

static void sf_opaque_terminate_c4_EKF(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc4_EKFInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc4_EKF((SFc4_EKFInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c4_EKF(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c4_EKF((SFc4_EKFInstanceStruct*)(((ChartInfoStruct *)
      ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c4_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"EKF","EKF",4);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"EKF","EKF",4,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"EKF","EKF",4,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"EKF","EKF",4,4);
      sf_mark_chart_reusable_outputs(S,"EKF","EKF",4,1);
    }

    sf_set_rtw_dwork_info(S,"EKF","EKF",4);
...
static void mdlRTW_c4_EKF(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "EKF", "EKF",4);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c4_EKF(SimStruct *S)
{
  SFc4_EKFInstanceStruct *chartInstance;
  chartInstance = (SFc4_EKFInstanceStruct *)malloc(sizeof(SFc4_EKFInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc4_EKFInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c4_EKF;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c4_EKF;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c4_EKF;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c4_EKF;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c4_EKF;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c4_EKF;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c4_EKF;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c4_EKF;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c4_EKF;
  chartInstance->chartInfo.mdlStart = mdlStart_c4_EKF;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c4_EKF;
...
void c4_EKF_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   ...
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c4_EKF(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c4_EKF(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c4_EKF_method_dispatcher.\n"
