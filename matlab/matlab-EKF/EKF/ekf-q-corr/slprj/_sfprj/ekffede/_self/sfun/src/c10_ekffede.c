...
static void initialize_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void initialize_params_c10_ekffede(SFc10_ekffedeInstanceStruct
  *chartInstance);
static void enable_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void disable_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void c10_update_debugger_state_c10_ekffede(SFc10_ekffedeInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c10_ekffede(SFc10_ekffedeInstanceStruct
  *chartInstance);
static void set_sim_state_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_st);
static void finalize_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void sf_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void initSimStructsc10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c10_machineNumber, uint32_T
  c10_chartNumber);
static const mxArray *c10_sf_marshallOut(void *chartInstanceVoid, void
  *c10_inData);
static real_T c10_emlrt_marshallIn(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_wave, const char_T *c10_identifier);
static real_T c10_b_emlrt_marshallIn(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_u, const emlrtMsgIdentifier *c10_parentId);
static void c10_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c10_mxArrayInData, const char_T *c10_varName, void *c10_outData);
static const mxArray *c10_b_sf_marshallOut(void *chartInstanceVoid, void
  *c10_inData);
static int32_T c10_c_emlrt_marshallIn(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_u, const emlrtMsgIdentifier *c10_parentId);
static void c10_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c10_mxArrayInData, const char_T *c10_varName, void *c10_outData);
static uint8_T c10_d_emlrt_marshallIn(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_b_is_active_c10_ekffede, const char_T *c10_identifier);
static uint8_T c10_e_emlrt_marshallIn(SFc10_ekffedeInstanceStruct *chartInstance,
  const mxArray *c10_u, const emlrtMsgIdentifier *c10_parentId);
static void init_dsm_address_info(SFc10_ekffedeInstanceStruct *chartInstance);
...
static void initialize_c10_ekffede(SFc10_ekffedeInstanceStruct *chartInstance)
{
  ...
  chartInstance->c10_is_active_c10_ekffede = 0U;
}
  ...
  *chartInstance)
{
  ...
  c10_b_hoistedGlobal = chartInstance->c10_is_active_c10_ekffede;
  ...
  const mxArray *c10_st)
{
  ...
  *c10_wave = c10_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell
    (c10_u, 0)), "wave");
  chartInstance->c10_is_active_c10_ekffede = c10_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c10_u, 1)),
     "is_active_c10_ekffede");
  sf_mex_destroy(&c10_u);
  c10_update_debugger_state_c10_ekffede(chartInstance);
  ...
  *c10_inData)
{
  ...
  real_T c10_u;
  const mxArray *c10_y = NULL;
  SFc10_ekffedeInstanceStruct *chartInstance;
  chartInstance = (SFc10_ekffedeInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c10_ekffede_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c10_b_is_active_c10_ekffede, const char_T *c10_identifier)
{
  ...
  c10_y = c10_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c10_b_is_active_c10_ekffede), &c10_thisId);
  sf_mex_destroy(&c10_b_is_active_c10_ekffede);
  return c10_y;
}
...
/* SFunction Glue Code */
void sf_c10_ekffede_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c10_ekffede_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c10_ekffede(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"wave\",},{M[8],M[0],T\"is_active_c10_ekffede\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c10_ekffede_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc10_ekffedeInstanceStruct *chartInstance;
    chartInstance = (SFc10_ekffedeInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c10_ekffede(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc10_ekffedeInstanceStruct*) chartInstanceVar)
    ->S,0);
  initialize_params_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
  initialize_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c10_ekffede(void *chartInstanceVar)
{
  enable_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c10_ekffede(void *chartInstanceVar)
{
  disable_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c10_ekffede(void *chartInstanceVar)
{
  sf_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c10_ekffede(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c10_ekffede((SFc10_ekffedeInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c10_ekffede();/* state var info */
...
extern void sf_internal_set_sim_state_c10_ekffede(SimStruct* S, const mxArray
  *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c10_ekffede((SFc10_ekffedeInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c10_ekffede(SimStruct* S)
{
  return sf_internal_get_sim_state_c10_ekffede(S);
}

static void sf_opaque_set_sim_state_c10_ekffede(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c10_ekffede(S, st);
}

static void sf_opaque_terminate_c10_ekffede(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc10_ekffedeInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc10_ekffede((SFc10_ekffedeInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c10_ekffede(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c10_ekffede((SFc10_ekffedeInstanceStruct*)
      (((ChartInfoStruct *)ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c10_ekffede(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"ekffede","ekffede",10);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"ekffede","ekffede",10,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"ekffede","ekffede",10,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"ekffede","ekffede",10,5);
      sf_mark_chart_reusable_outputs(S,"ekffede","ekffede",10,1);
    }

    sf_set_rtw_dwork_info(S,"ekffede","ekffede",10);
...
static void mdlRTW_c10_ekffede(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "ekffede", "ekffede",10);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c10_ekffede(SimStruct *S)
{
  SFc10_ekffedeInstanceStruct *chartInstance;
  chartInstance = (SFc10_ekffedeInstanceStruct *)malloc(sizeof
    (SFc10_ekffedeInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc10_ekffedeInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c10_ekffede;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c10_ekffede;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c10_ekffede;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c10_ekffede;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c10_ekffede;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c10_ekffede;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c10_ekffede;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c10_ekffede;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c10_ekffede;
  chartInstance->chartInfo.mdlStart = mdlStart_c10_ekffede;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c10_ekffede;
...
void c10_ekffede_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c10_ekffede(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c10_ekffede(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c10_ekffede(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c10_ekffede_method_dispatcher.\n"
