...
static void initialize_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void initialize_params_c8_ekffede(SFc8_ekffedeInstanceStruct
  *chartInstance);
static void enable_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void disable_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void c8_update_debugger_state_c8_ekffede(SFc8_ekffedeInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c8_ekffede(SFc8_ekffedeInstanceStruct
  *chartInstance);
static void set_sim_state_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_st);
static void finalize_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void sf_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void initSimStructsc8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c8_machineNumber, uint32_T
  c8_chartNumber);
static const mxArray *c8_sf_marshallOut(void *chartInstanceVoid, void *c8_inData);
static real_T c8_emlrt_marshallIn(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_wave, const char_T *c8_identifier);
static real_T c8_b_emlrt_marshallIn(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_u, const emlrtMsgIdentifier *c8_parentId);
static void c8_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c8_mxArrayInData, const char_T *c8_varName, void *c8_outData);
static const mxArray *c8_b_sf_marshallOut(void *chartInstanceVoid, void
  *c8_inData);
static int32_T c8_c_emlrt_marshallIn(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_u, const emlrtMsgIdentifier *c8_parentId);
static void c8_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c8_mxArrayInData, const char_T *c8_varName, void *c8_outData);
static uint8_T c8_d_emlrt_marshallIn(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_b_is_active_c8_ekffede, const char_T *c8_identifier);
static uint8_T c8_e_emlrt_marshallIn(SFc8_ekffedeInstanceStruct *chartInstance,
  const mxArray *c8_u, const emlrtMsgIdentifier *c8_parentId);
static void init_dsm_address_info(SFc8_ekffedeInstanceStruct *chartInstance);
...
static void initialize_c8_ekffede(SFc8_ekffedeInstanceStruct *chartInstance)
{
  ...
  chartInstance->c8_is_active_c8_ekffede = 0U;
}
  ...
  *chartInstance)
{
  ...
  c8_b_hoistedGlobal = chartInstance->c8_is_active_c8_ekffede;
  ...
  const mxArray *c8_st)
{
  ...
  *c8_wave = c8_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c8_u,
    0)), "wave");
  chartInstance->c8_is_active_c8_ekffede = c8_d_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c8_u, 1)), "is_active_c8_ekffede");
  sf_mex_destroy(&c8_u);
  c8_update_debugger_state_c8_ekffede(chartInstance);
...
static const mxArray *c8_sf_marshallOut(void *chartInstanceVoid, void *c8_inData)
{
  ...
  real_T c8_u;
  const mxArray *c8_y = NULL;
  SFc8_ekffedeInstanceStruct *chartInstance;
  chartInstance = (SFc8_ekffedeInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c8_ekffede_get_eml_resolved_functions_info(void)
{
  ...
  const mxArray *c8_b_is_active_c8_ekffede, const char_T *c8_identifier)
{
  ...
  c8_y = c8_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c8_b_is_active_c8_ekffede), &c8_thisId);
  sf_mex_destroy(&c8_b_is_active_c8_ekffede);
  return c8_y;
}
...
/* SFunction Glue Code */
void sf_c8_ekffede_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c8_ekffede_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c8_ekffede(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"wave\",},{M[8],M[0],T\"is_active_c8_ekffede\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c8_ekffede_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc8_ekffedeInstanceStruct *chartInstance;
    chartInstance = (SFc8_ekffedeInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c8_ekffede(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc8_ekffedeInstanceStruct*) chartInstanceVar)->S,
    0);
  initialize_params_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
  initialize_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c8_ekffede(void *chartInstanceVar)
{
  enable_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c8_ekffede(void *chartInstanceVar)
{
  disable_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c8_ekffede(void *chartInstanceVar)
{
  sf_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c8_ekffede(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c8_ekffede((SFc8_ekffedeInstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c8_ekffede();/* state var info */
...
extern void sf_internal_set_sim_state_c8_ekffede(SimStruct* S, const mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c8_ekffede((SFc8_ekffedeInstanceStruct*)chartInfo->chartInstance,
    mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c8_ekffede(SimStruct* S)
{
  return sf_internal_get_sim_state_c8_ekffede(S);
}

static void sf_opaque_set_sim_state_c8_ekffede(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c8_ekffede(S, st);
}

static void sf_opaque_terminate_c8_ekffede(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc8_ekffedeInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc8_ekffede((SFc8_ekffedeInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c8_ekffede(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c8_ekffede((SFc8_ekffedeInstanceStruct*)(((ChartInfoStruct
      *)ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c8_ekffede(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"ekffede","ekffede",8);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"ekffede","ekffede",8,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"ekffede","ekffede",8,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"ekffede","ekffede",8,5);
      sf_mark_chart_reusable_outputs(S,"ekffede","ekffede",8,1);
    }

    sf_set_rtw_dwork_info(S,"ekffede","ekffede",8);
...
static void mdlRTW_c8_ekffede(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "ekffede", "ekffede",8);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c8_ekffede(SimStruct *S)
{
  SFc8_ekffedeInstanceStruct *chartInstance;
  chartInstance = (SFc8_ekffedeInstanceStruct *)malloc(sizeof
    (SFc8_ekffedeInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc8_ekffedeInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c8_ekffede;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c8_ekffede;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c8_ekffede;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c8_ekffede;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c8_ekffede;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c8_ekffede;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c8_ekffede;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c8_ekffede;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c8_ekffede;
  chartInstance->chartInfo.mdlStart = mdlStart_c8_ekffede;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c8_ekffede;
...
void c8_ekffede_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c8_ekffede(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c8_ekffede(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c8_ekffede(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c8_ekffede_method_dispatcher.\n"
